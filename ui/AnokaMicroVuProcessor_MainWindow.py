...
from ui.Anokagui_MicroVuProcessor_MainWindow import Anokagui_MicroVuProcessorMainWindow
...
class MicroVuProcessorMainWindow(QtWidgets.QMainWindow, Anokagui_MicroVuProcessorMainWindow):
    ...
    def load_micro_vus(self) -> None:
        ...
        for row in range(self.tableWidget.rowCount()):
            ...
            op_number = self.tableWidget.item(row, 4).text()
            rev_number = self.tableWidget.item(row, 5).text()
            micro_vu = MicroVuProgram(input_filepath, op_number, rev_number, smartprofile_file_name)
    ...
    def load_table_widget(self) -> None:
        ...
        if not os.path.exists(self.txtInputFolder.text()):
            ...
            for row, file in enumerate(files):
                ...
                op_textItem = QTableWidgetItem("")
                self.tableWidget.setItem(row, 4, op_textItem)
                rev_textItem = QTableWidgetItem("")
                self.tableWidget.setItem(row, 5, rev_textItem)
